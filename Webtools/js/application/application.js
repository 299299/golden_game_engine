...
// Public:
    ...
    this.resetOptions = function() {
        application.options = {
            autoConnect: true,
            autoConnectServer: 'localhost:6161',
...
function send_console_command(cmd, data1, data2) {
    var value = {'command':cmd, 'data1':data1, 'data2':data2};
    application.send('remote.command', value);
}
function help() {
    application.log('The avaiable command is : ');
    application.log('       clear_scene()');
    application.log('       add_entity(name)');
    application.log('       reload_resource(type, name)');
    application.log('       list_resources(type)');
}
function clear_scene() {
    send_console_command("clear_scene");
}
function add_entity(name) {
    send_console_command("add_entity", name);
}
function reload_resource(type, name) {
    send_console_command("reload_resource", type, name);
}
function list_resources(type) {
    send_console_command("list_resources", type);
}