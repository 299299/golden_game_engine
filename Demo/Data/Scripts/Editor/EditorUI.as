...
uint timeScaleIndex = 0;
Array<float> timeScales = {0.1, 0.25, 0.5, 1.0, 2, 4, 10};
...
void HandleKeyDown(StringHash eventType, VariantMap& eventData)
{
    ...
    else if (eventData["Qualifiers"].GetInt() == QUAL_CTRL)
    {
        ...
        {
            scene.timeScale = timeScales[timeScaleIndex];
            ++timeScaleIndex;
...
{
    int nWheel = eventData["Wheel"].GetInt();
    UIElement@ elem = ui.focusElement;
    if(elem is null)
        return;
    LineEdit@ edit = cast<LineEdit>(elem);
    if(edit is null)
        return;
    if(!edit.vars.Contains("Coordinate"))
        return;
    float fValue = edit.text.ToFloat();
    fValue += nWheel * 0.1f;
